...
export class CovSubRenderController {
  ...
  private _lastPoppedQueueItem: QueueItem | null | undefined = null;
  ...
  public setMode(mode: TranscriptHelperMode) {
    if (this._mode !== TranscriptHelperMode.UNKNOWN) {
      this.callMessagePrint(
        ELoggerType.warn,
  ...
  public handleMessage<T extends TranscriptionBase>(
    ...
    // set mode (only once)
    if (isAgentMessage && this._mode === TranscriptHelperMode.UNKNOWN) {
